...
function ShowHideHandler( isHide, bInit )
	...
	if not Matchmaking.IsLaunchingGame() and not isHide then 
		...
		if(bIsModding) then
			...
			-- Show mods button in regular multiplayer too
			Controls.ModsButton:SetHide( false );
		end
		...
		-- Refresh mods display for multiplayer
		RefreshModsDisplay();
...
----------------------------------------------------------------        
-- Refresh Mods Display
----------------------------------------------------------------        
function RefreshModsDisplay()
	...
	if modCount > 0 then
		...
		for i, mod in ipairs(activeMods) do
			local modName = Modding.GetModProperty(mod.ID, mod.Version, "Name") or "Unknown Mod"
			table.insert(modNames, modName)
		end
		
		local modText = table.concat(modNames, ", ")
		if string.len(modText) > 60 then
			modText = string.sub(modText, 1, 57) .. "..."
		end
		
		Controls.ModsHelp:SetText(string.format("%d mods active: %s", modCount, modText))
	else
		Controls.ModsHelp:SetText("No mods active")
	end
end
